...
/**
 * The marker hand on the "coming soon" sticky note, and nothing else. Loaded
 * here rather than in the component so it goes through next/font like Manrope
 * — self-hosted, preloaded, no layout shift and no runtime request to Google.
 * One weight is all the face ships.
 */
...
/**
 * The script hand on the "coming soon" note's title. Same self-hosting
 * rationale as the marker face above.
 */
const caveat = Caveat({
  subsets: ["latin"],
  weight: ["500", "600"],
  variable: "--font-caveat-face",
  display: "swap",
});
...
export default async function RootLayout({
  ...
  // `suppressHydrationWarning` on <html> and <body> below is load-bearing, not
  // boilerplate: the arming script stamps a class on <html> before React
  // hydrates, so the client's className cannot match the server HTML. Removing
  // either attribute means a hydration warning on every armed load of the home
  // page.
  return (
    <html
      lang="en"
      className={`h-full antialiased ${manrope.variable} ${marker.variable} ${caveat.variable}`}
